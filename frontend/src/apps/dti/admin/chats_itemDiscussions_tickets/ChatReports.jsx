...
// Main Component
// ============================================================
const ChatReports = ({ embedded = false }) => {
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      ...
      const response = await chatAPI.adminGetReports(params);
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      await chatAPI.adminUpdateStatus(selectedReport.id, newStatus);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await chatAPI.adminDelete(deleteReport.id);
  ...
  const handleOpenChat = async (report) => {
    ...
    try {
      ...
      const otherUser = report.reporter || { full_name: 'Unknown' };
      const item = {
        id: conv.item_id || null,
        title: 'Conversation',
      };
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-ink-500">
                {stats.total} total reports · Reports from direct conversations
              </p>
            </div>
          </div>
        )}
        ...
        {filteredReports.length === 0 ? (
          <div className="relative overflow-hidden rounded-2xl border border-ink-100/80 bg-white p-12 text-center shadow-sm">
            ...
            <div className="relative">
              ...
              <h3 className="text-lg font-extrabold text-ink-800">No chat reports found</h3>
              <p className="mx-auto mt-1 max-w-md text-sm text-ink-400">
                {filter !== 'all' || search
                  ? 'Try adjusting your filters or search term.'
                  : 'No chat reports available right now.'}
              </p>
            </div>
          </div>
        ) : viewMode === 'cards' ? (
          ...
          >
            {paginatedReports.map((report) => {
              const conv = report.conversation || {};
              const participants = [];
              if (conv.donor_id) participants.push(conv.donor_id);
              if (conv.applicant_id) participants.push(conv.applicant_id);
              const participantNames = participants.join(', ') || 'Unknown';
              const isOpening = openingChatId === report.id;
                ...
                >
                  <div className="p-4 space-y-3">
                    ...
                    {report.description && (
                      <div>
                        ...
                        <p
                          className="mt-0.5 text-sm text-ink-600 line-clamp-2 leading-relaxed"
                    ...
                    {/* Conversation */}
                    {conv.id && (
                      <div className="flex items-center justify-between text-xs font-medium text-ink-500 bg-ink-50/50 rounded-xl p-2 border border-ink-100/60">
                        <span className="flex items-center gap-1.5 truncate min-w-0" title={participantNames}>
                  ...
                  <div className="border-t border-ink-100 bg-ink-50/50 px-4 py-3 flex items-center justify-between gap-2 rounded-b-2xl">
                    {conv.id ? (
                      ...
                      <span className="text-xs text-ink-400 italic">No conversation</span>
                    )}
                    ...
                    return (
                      <tr key={report.id} className="hover:bg-primary-50/20 transition group">
                        ...
                        <td className="py-3.5 px-4 max-w-[160px]">
                          {conv.id ? (
                            <button
                              onClick={() => handleOpenChat(report)}
                              disabled={isOpening}
                              className="text-ink-600 hover:text-primary-600 transition truncate block font-medium text-left disabled:opacity-50"
                              title={`View conversation between ${participantNames}`}
                            >
                              <span className="inline-flex items-center gap-1">
                                ...
                                <span className="truncate max-w-[120px]">{participantNames}</span>
                              </span>
                            </button>
                          ) : (
                            <span className="text-ink-400 italic">N/A</span>
                          )}
                        </td>
                        ...
                        <td className="py-3.5 px-4 max-w-[180px]">
                          {report.description ? (
                            <span className="truncate block" title={report.description}>
                              {report.description}
                            </span>
                          ) : (
                          ...
                          <div className="flex items-center justify-end gap-1.5">
                            {conv.id && (
                              <button
                                onClick={() => handleOpenChat(report)}
                                disabled={isOpening}
                                className="px-2.5 py-1 bg-primary-50 text-primary-700 hover:bg-primary-100 rounded-lg text-xs font-bold transition flex items-center gap-1 border border-primary-200 disabled:opacity-50"
                                title="View Chat"
                              >
                                {isOpening ? (
